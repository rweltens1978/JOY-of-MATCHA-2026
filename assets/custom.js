        ...
        .then((response) => {
            ...
            else
            {
                if(response.id != "undefined" && response.id != null && response.id != "")
                {
                    ...
                    // Refresh CartJS cart drawer (fix for theme editor issue)
                    refreshCartJS();
                }
            }
        })
...
// Function to refresh CartJS cart drawer (fix for theme editor issue)
function refreshCartJS() {
  if (typeof CartJS !== 'undefined' && typeof CartJS.getCart === 'function') {
    CartJS.getCart({
      success: (cart) => {
        ...
        // Update cart icon count (fix for cart icon update issue)
        ...
        if (cartSlide && typeof cartSlide.updateCartIconCount === 'function') {
          ...
          // Fallback: update cart icon directly
        ...
        // Also refresh cart drawer if it exists
        if (cartSlide && typeof cartSlide.refreshCart === 'function') {
          cartSlide.refreshCart();
        }
        
        // In theme editor, ensure cart drawer opens and displays products (fix for theme editor PDP issue)
        // Use a longer delay in theme editor to ensure CartJS and Rivets.js have time to update
        const delay = (typeof Shopify !== 'undefined' && Shopify.designMode) ? 400 : 200;
        setTimeout(() => {
          if (cartSlide) {
            // Force refresh cart again to ensure Rivets.js updates (theme editor fix)
            if (typeof cartSlide.refreshCart === 'function') {
              cartSlide.refreshCart();
            }
            // Open cart drawer if it's not already open (theme editor fix)
      ...
      error: () => {
        // Silently fail if CartJS is not available
      }
    });
  } else {
    // Fallback: if CartJS is not available, try to update cart icon directly
    fetch('/cart.js')
      .then(response => response.json())
      .then(cart => {
        updateCartIconDirectly(cart.item_count || 0);
...
function ajaxCartCall()
{
    ...
    .then(response => response.json())
        .then((response) => {
            ...
            // Update cart-count-bubble (fix for cart icon update issue)
            ...
            // Update cart link bubble visibility
            ...
            // Also update via cart-slide if available (fix for cart icon update issue)
            const cartSlide = document.querySelector('cart-slide');
            if (cartSlide && typeof cartSlide.updateCartIconCount === 'function') {
                cartSlide.updateCartIconCount(itemCount);
            }
        })
        .catch(function(error) {
        });
        ...
        productVariantSelect.forEach(function (topel, topkey) {
            topel.addEventListener('change', function(event) {
                ...
                if(current.parentElement.parentElement.parentElement.classList.contains('product__detail')) // collection and home page product lists
                {
                    ...
                    // Fix: data-sale-price is the current price, data-old-price is compare_at_price
                    var variantOriginalPrice = this.options[this.selectedIndex].getAttribute('data-sale-price'); // Current price
                    var variantSalePrice = this.options[this.selectedIndex].getAttribute('data-old-price'); // Compare at price (if on sale)
